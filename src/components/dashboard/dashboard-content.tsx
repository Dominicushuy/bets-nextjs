...
export default function DashboardContent({ userId }: DashboardContentProps) {
  ...
  const { data: activeGamesData, isLoading: gamesLoading } =
    ...
    <div className='space-y-6'>
      ...
      <div className='grid grid-cols-1 lg:grid-cols-3 gap-6'>
        <div className='lg:col-span-2 space-y-6'>
          <StatisticsWidget userId={userId} />
          <FeaturedGames />
        </div>
        <div className='space-y-6'>
          <ActivityFeed />
      ...
      {/* Thống kê biểu đồ nếu có dữ liệu */}
      {userBets && userBets.length > 0 && statistics && (
        <div className='mt-6'>
          <StatisticsCharts betsData={userBets} userStatistics={statistics} />
        </div>
      )}
